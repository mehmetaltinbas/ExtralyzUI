...
export function TrueFalseExerciseEvaluationCard({ exercise, evaluation, index }: {
    exercise: Exercise;
    evaluation: ExerciseAnswerEvaluationResult;
    index: number;
}) {
    const userAnswer = Number(evaluation.userAnswer) === 1 ? 'true' : (Number(evaluation.userAnswer) === 0 ? 'false' : 'empty');
    const correctAnswer = exercise.correctChoiceIndex === 1 ? 'true' : (exercise.correctChoiceIndex === 0 ? 'false' : undefined);
